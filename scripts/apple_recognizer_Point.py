...
MODEL_PATH = f"/home/emanuele/Documents/emanuelearca/runs_v{YOLO_VERSION}/detect/train/weights/best.pt"  # Percorso al modello YOLO
...
class RealSenseViewer(Node):
    def __init__(self):
        ...
        self.apple_coordinates_publisher = self.create_publisher(Point, 'apple_coordinates', 10)
    ...
    def publish_apple_coordinates(self, coordinates):
        ...
        """
        try:
            # Crea e pubblica il messaggio Point
            point = Point()
            point.x, point.y, point.z = map(float,coordinates)
            self.apple_coordinates_publisher.publish(point)
...
def print_coordinates(coordinates_list, confidences, first_sended):
    ...
    for i, ((Xcam, Ycam, Zcam), confidence) in enumerate(zip(coordinates_list, confidences), start=1):
        ...
        print(f"{i}) X: {Xcam:.1f} Y: {Ycam:.1f} Z: {Zcam:.1f} {confidence:.1f}%{sended_marker}")
