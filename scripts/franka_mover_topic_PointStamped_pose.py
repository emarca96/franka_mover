...
class MoveFR3(Node):
    ...
    def apple_callback(self, msg: PointStamped):
        """Callback per il topic /apple_coordinates."""
        self.get_logger().info(f"Received apple coordinates: x={msg.point.x}, y={msg.point.y}, z={msg.point.z}")
    ...
    def send_goal(self):
        ...
        if not self.target_point:
            self.get_logger().warn('No target point received yet. Waiting...')
            return
        ...
        target_pose.header.frame_id = self.target_point.header.frame_id
        target_pose.pose.position.x = self.target_point.point.x
        target_pose.pose.position.y = self.target_point.point.y
        target_pose.pose.position.z = self.target_point.point.z

        # Quaternione della posa (esempio: orientamento fisso)
